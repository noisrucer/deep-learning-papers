'''
Modifications to the original paper design

[1] Used padding=1 for double_conv to match the dimension. The final output has
the same spatial dimension as the input.

[2] Inserted Batch Normalization for double conv

[3] Not used cropping for skip connections. Instead resized the expansive path
features to match the contracting path features.
'''

import torch
import torch.nn as nn
from utils import crop_img, concat_imgs_resize
...
class UNet(nn.Module):
    '''
    [1] Padding 1 for double conv
    [2] Removed cropping -> resized ups to downs to get the original dimension
    '''
